...
const V = 1;
const TYPES = ['org', 'el', 'cand', 'stn', 'obs', 'res', 'disp', 'dres', 'omem', 'orem', 'ovis', 'odel', 'ecl', 'oedit', 'oxfer', 'oxacc'];
...
// Close an election (owner-only, locks structural changes).
...
function editOrgMemo(org, name, jur) {
  return { app: APP, v: V, t: 'oedit', org: String(org || ''), name: clampStr(name, 120), jur: clampStr(jur, 80) };
}
// Offer ownership transfer to another wallet. Owner only. Latest-wins per org.
function transferOfferMemo(org, to) {
  return { app: APP, v: V, t: 'oxfer', org: String(org || ''), to: String(to || '') };
}
// Accept a pending ownership offer. Must come from the pendingOwner wallet.
function transferAcceptMemo(org) {
  return { app: APP, v: V, t: 'oxacc', org: String(org || '') };
}
...
function decode(str) {
  ...
  switch (o.t) {
    ...
    case 'oedit': {
      if (!o.org || !o.name || typeof o.name !== 'string' || !o.name.trim()) return null;
      return { app: APP, v: V, t: 'oedit', org: String(o.org), name: clampStr(o.name, 120), jur: clampStr(o.jur, 80) };
    }
    case 'oxfer': {
      if (!o.org || !o.to) return null;
      return { app: APP, v: V, t: 'oxfer', org: String(o.org), to: String(o.to) };
    }
    case 'oxacc': {
      if (!o.org) return null;
      return { app: APP, v: V, t: 'oxacc', org: String(o.org) };
    }
    default:
      return null;
  }
...
module.exports = {
  ...
  memberMemo, removeMemberMemo, visibilityMemo, deleteOrgMemo, electionCloseMemo, editOrgMemo,
  transferOfferMemo, transferAcceptMemo,
