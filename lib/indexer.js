...
class QuickCountIndexer {
  ...
  reset() {
    this.orgs = new Map();        // addr -> { addr, name, jur, feeAmount, active, visibility, deleted, ownerAddr, pendingOwner, members:Map<addr,role>, txId, createdAt }
    ...
    this.auditEntries = new Map(); // orgAddr -> [{ action, actor, target, detail, txId, createdAt }]
  }

  // Returns whether wallet is the current Owner of orgAddr (by ownerAddr, falling
  // back to founding addr for pre-transfer orgs). Deleted orgs have no owner.
  isOwner(orgAddr, wallet) {
    if (!orgAddr || !wallet) return false;
    const org = this.orgs.get(orgAddr);
    if (!org) return false;
    return wallet === (org.ownerAddr || org.addr);
  }

  // Resolve a wallet's role within an org. The ownerAddr wallet (defaulting to the
  // founding wallet org.addr for pre-transfer orgs) is the implicit Owner — always
  // highest priority, never stored in `members`. Returns one of
  ...
  orgRole(orgAddr, wallet) {
    ...
    if (wallet === (org.ownerAddr || org.addr)) return 'owner';
  ...
  _pushAudit(orgAddr, entry) {
    let entries = this.auditEntries.get(orgAddr);
    if (!entries) { entries = []; this.auditEntries.set(orgAddr, entries); }
    entries.push(entry);
  }
  ...
  apply(tx) {
    ...
    switch (env.t) {
      ...
      case 'omem': return this._member(tx, env, at);
      case 'orem': return this._removeMember(tx, env, at);
      case 'ovis': return this._visibility(tx, env, at);
      case 'odel': return this._deleteOrg(tx, env, at);
      case 'ecl': return this._closeElection(tx, env);
      case 'oedit': return this._editOrg(tx, env, at);
      case 'oxfer': return this._transferOffer(tx, env, at);
      case 'oxacc': return this._transferAccept(tx, env, at);
  ...
  _org(tx, env, at) {
    ...
    if (!existing) {
      this.orgs.set(addr, {
        ...
        ownerAddr: addr, pendingOwner: null,
  ...
  _member(tx, env, at) {
    ...
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (org.deleted) return { applied: false, reason: 'org-deleted' };
    // Protect the current Owner from being targeted as a member.
    if (this.isOwner(org.addr, env.addr)) return { applied: false, reason: 'owner-immutable' };
    const actor = this.orgRole(org.addr, tx.from);
    if (actor === 'owner') {
      org.members.set(env.addr, env.role);
      if (env.role === 'admin') {
        this._pushAudit(org.addr, { action: 'admin_assigned', actor: tx.from, target: env.addr, detail: null, txId: tx.txId, createdAt: at });
      }
  ...
  _removeMember(tx, env, at) {
    const org = this.orgs.get(env.org);
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (org.deleted) return { applied: false, reason: 'org-deleted' };
    // Protect the current Owner from being removed.
    if (this.isOwner(org.addr, env.addr)) return { applied: false, reason: 'owner-immutable' };
    const actor = this.orgRole(org.addr, tx.from);
    if (actor === 'owner') {
      const removedRole = org.members.get(env.addr) || null;
      org.members.delete(env.addr);
      this._pushAudit(org.addr, { action: 'member_removed', actor: tx.from, target: env.addr, detail: removedRole, txId: tx.txId, createdAt: at });
  ...
  _visibility(tx, env, at) {
    ...
    this._pushAudit(org.addr, { action: 'visibility_changed', actor: tx.from, target: null, detail: env.vis, txId: tx.txId, createdAt: at });
    return { applied: true, kind: 'ovis', vis: env.vis };
  }

  // Delete (tombstone) the org. Owner only.
  _deleteOrg(tx, env, at) {
    const org = this.orgs.get(env.org);
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (this.orgRole(org.addr, tx.from) !== 'owner') return { applied: false, reason: 'unauthorized' };
    this._pushAudit(org.addr, { action: 'org_deleted', actor: tx.from, target: null, detail: null, txId: tx.txId, createdAt: at });
    org.deleted = true;
  ...
  _editOrg(tx, env, at) {
    ...
    org.name = name;
    org.jur = env.jur || '';
    this._pushAudit(org.addr, { action: 'org_edited', actor: tx.from, target: null, detail: name, txId: tx.txId, createdAt: at });
    return { applied: true, kind: 'oedit' };
  }

  // Offer ownership transfer to another wallet. Owner only. Latest-wins.
  // Offering to the current owner address cancels any pending offer.
  _transferOffer(tx, env, at) {
    const org = this.orgs.get(env.org);
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (org.deleted) return { applied: false, reason: 'org-deleted' };
    if (!this.isOwner(org.addr, tx.from)) return { applied: false, reason: 'unauthorized' };
    // Offering to current owner clears any pending offer (cancel gesture).
    if (this.isOwner(org.addr, env.to)) {
      org.pendingOwner = null;
      return { applied: true, kind: 'oxfer', cancelled: true };
    }
    org.pendingOwner = env.to;
    this._pushAudit(org.addr, { action: 'ownership_offered', actor: tx.from, target: env.to, detail: null, txId: tx.txId, createdAt: at });
    return { applied: true, kind: 'oxfer' };
  }

  // Accept a pending ownership offer. Must come from pendingOwner wallet.
  // On success: new wallet becomes Owner; old Owner is downgraded to admin.
  _transferAccept(tx, env, at) {
    const org = this.orgs.get(env.org);
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (org.deleted) return { applied: false, reason: 'org-deleted' };
    if (!org.pendingOwner || tx.from !== org.pendingOwner) return { applied: false, reason: 'unauthorized' };
    const oldOwner = org.ownerAddr || org.addr;
    org.ownerAddr = tx.from;
    org.members.delete(tx.from); // new owner is never stored in members
    org.members.set(oldOwner, 'admin'); // demote previous owner to admin
    org.pendingOwner = null;
    this._pushAudit(org.addr, { action: 'ownership_transferred', actor: tx.from, target: oldOwner, detail: null, txId: tx.txId, createdAt: at });
    return { applied: true, kind: 'oxacc', newOwner: tx.from, oldOwner };
  }

  // ── Snapshots (visibility-aware) ─────────────────────────────────────────

  // Which elections are visible to a viewer: any active org's elections are
  // public; the org owner additionally sees its own (pending) elections.
  // Pass admin: true to bypass visibility restrictions (platform break-glass).
  visibleElections({ viewer, admin } = {}) {
    ...
    for (const el of this.elections.values()) {
      ...
      const isOwner = viewer && this.orgRole(el.orgAddr, viewer) === 'owner';
      if (!(org.active || isOwner || admin)) continue;
      // Private orgs are visible only to members (any role) or admins.
      if (org.visibility === 'private' && !this.orgRole(org.addr, viewer) && !admin) continue;
      out.push(el);
    }
  ...
  viewerRole(viewer) {
    const ownedOrgs = this._ownedOrgs(viewer);
    const ownOrg = ownedOrgs[0] || null;
    const observerOf = [];
    if (viewer) {
    ...
    return {
      ...
      observerOf,
      orgsOwned: ownedOrgs.map((o) => this._orgCard(o, 'owner')),
      orgsMember: this._memberOrgs(viewer),
      orgsPendingOwner: this._pendingOwnerOrgs(viewer),
    };
  }
  ...
  // Orgs where this viewer is the current Owner (by ownerAddr).
  _ownedOrgs(viewer) {
    const out = [];
    if (!viewer) return out;
    for (const org of this.orgs.values()) {
      if (org.deleted) continue;
      if ((org.ownerAddr || org.addr) === viewer) out.push(org);
    }
    return out;
  }

  // Orgs (excluding owned ones) where the wallet holds a role.
  _memberOrgs(viewer) {
    const out = [];
    if (!viewer) return out;
    for (const org of this.orgs.values()) {
      if (org.deleted) continue;
      if ((org.ownerAddr || org.addr) === viewer) continue; // owned org lives in orgsOwned
  ...
  // Orgs where this viewer has been offered ownership but not yet accepted.
  _pendingOwnerOrgs(viewer) {
    const out = [];
    if (!viewer) return out;
    for (const org of this.orgs.values()) {
      if (org.deleted) continue;
      if (org.pendingOwner === viewer) out.push(this._orgCard(org, null));
    }
    return out;
  }

  // Full org detail for one org as seen by `viewer` — card + roster + audit.
  // Returns null when the org is unknown/deleted or the viewer has no relation to it.
  orgDetail(orgAddr, viewer) {
    const org = this.orgs.get(orgAddr);
    if (!org || org.deleted) return null;
    const viewerRole = this.orgRole(org.addr, viewer);
    const isPendingOwner = viewer && org.pendingOwner === viewer;
    if (!viewerRole && !isPendingOwner) return null;

    const ownerAddr = org.ownerAddr || org.addr;
    const members = [{ addr: ownerAddr, role: 'owner' }];
    for (const [addr, role] of org.members) members.push({ addr, role });

    const audit = (viewerRole === 'owner' || viewerRole === 'admin')
      ? (this.auditEntries.get(orgAddr) || []).slice().reverse()
      : undefined;
    ...
    return {
      ...
      pendingOwner: org.pendingOwner || null,
      members: viewerRole ? members : [],
      audit,
    };
  }

  // Owned + member-of + pending-owner orgs for a viewer, each with its roster.
  orgsForViewer(viewer) {
    const role = this.viewerRole(viewer);
    const cards = [...role.orgsOwned, ...role.orgsMember, ...role.orgsPendingOwner];
