...
class QuickCountIndexer {
  ...
  _election(tx, env, at) {
    ...
    if (!this.elections.has(eid)) {
      this.elections.set(eid, { eid, name: env.name, orgAddr: org.addr, closed: false, txId: eid, createdAt: at });
  ...
  // Edit org name/jurisdiction. Owner only. Latest-wins.
  _editOrg(tx, env) {
    const org = this.orgs.get(env.org);
    if (!org) return { applied: false, reason: 'unknown-org' };
    if (org.deleted) return { applied: false, reason: 'org-deleted' };
    if (this.orgRole(org.addr, tx.from) !== 'owner') return { applied: false, reason: 'unauthorized' };
    if (env.name) org.name = env.name;
    if (env.jur != null) org.jur = env.jur;
    return { applied: true, kind: 'oedit' };
  }

  // Close an election (lock structural changes). Owner only.
  ...
  // public; an org member additionally sees elections of their (pending/private) org.
  visibleElections({ viewer } = {}) {
    const out = [];
    for (const el of this.elections.values()) {
      const org = this.orgs.get(el.orgAddr);
      if (!org || org.deleted) continue;
      const isOwner = viewer && el.orgAddr === viewer;
      if (!(org.active || isOwner || (viewer && this.orgRole(org.addr, viewer)))) continue;
  ...
  electionDetail(eid, method = 'latest') {
    ...
    return {
      election: { eid: el.eid, name: el.name, orgAddr: el.orgAddr, orgName: org ? org.name : null, orgActive: org ? org.active : false, orgJur: org ? (org.jur || '') : '', closed: !!el.closed, createdAt: el.createdAt },
      method,
  ...
  activityByAddr(addr, visibleEids, opts = {}) {
    ...
    const organizations = this.orgsForAddr(addr, opts.viewer || null, { admin: false });
